...
DELIMITER $$
CREATE PROCEDURE add_maestro(IN	nombre VARCHAR(30), IN apellido VARCHAR(30), IN no_registro VARCHAR(20), IN telefono VARCHAR(10), IN direccion VARCHAR(100), IN email VARCHAR(50), IN fecha_nac DATE , IN dpi VARCHAR(20), IN foto_perfil VARCHAR(100), IN contrasenia VARCHAR(20))
	BEGIN 
		INSERT INTO maestro (nombre, apellido, no_registro, telefono, direccion, email, fecha_nac , dpi, foto_perfil, contrasenia)
		VALUES (nombre, apellido, no_registro, telefono, direccion, email, fecha_nac , dpi, foto_perfil, contrasenia);
END$$ 
DELIMITER ;
